...
APP_APIURL=https://api.github.com/repos/oam-dev/kubevela
APP_APIURL=$APP_APIURL/releases/latest
APP_BASEURL=https://github.com/oam-dev/kubevela/releases/download
APP_BIN=vela
APP_PATH=~/bin/$APP_BIN
APP_VERSION=$(curl --silent $APP_APIURL | grep '"tag_name"' \
    | sed -E 's/.*"([^"]+)".*/\1/')
APP_SRC=vela-$APP_VERSION-$OS_TYPE
...
setup_darwin() {
    ...
    command -v brew > /dev/null && brew install kubevela
}
