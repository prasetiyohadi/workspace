...
APP_APIURL=https://api.github.com/repos/fujiapple852/trippy
APP_APIURL=$APP_APIURL/releases/latest
APP_BASEURL=https://github.com/fujiapple852/trippy/releases/download
APP_BIN=trip
APP_NAME=trippy
...
install() {
	mkdir -p ~/.local/bin
	sudo mv "/tmp/$APP_SRC/$APP_BIN" $APP_PATH
}
...
setup_linux() {
	echo "This script will install $APP_NAME version $APP_VERSION."
	if [ -s "$APP_PATH" ]; then
		check_version
