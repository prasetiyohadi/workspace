...
# Application details
APP_NAME=datadog
APP_NAMESPACE=datadog
APP_RELEASE=datadog
APP_REPO=datadog
APP_REPO_URL=https://helm.datadoghq.com
...
install() {
    ...
    if [ ! -f .env ]; then
        ...
        . .env && $HELM_CMD $HELM_SUBCMD -n $APP_NAMESPACE $APP_RELEASE $APP_REPO/$APP_NAME -f values.yaml --set datadog.apiKey=$DD_API_KEY
    fi
    popd &> /dev/null
}
